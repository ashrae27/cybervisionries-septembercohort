...
```
RAM in use: 15GB  out of total: 15.8GB
```
...
4. Process name: NVIDIA container :What I think it does: computer graphics for gaming
5. Process name: elan service :What I think it does: ethernet connection
```
...
**My Screenshot:** https://github.com/ashrae27/cybervisionries-septembercohort/blob/main/assets/screenshots/week-02/Task%20Manager.png
...
```
The number describes the hardware is the CPU and RAM; the network describes the OS.
```
...
```
The hardware runs on RAM, CPU, and storage. The hardware helps run the operating system.
```
...
```
Why watching the numbers changing in the CPU task manager to make sure the CPU number is running and still operating. The CPU can stop running by loosing power or the hardware is incorrect. Watching the CPU on task manager to make it won't lose power.
```
...
```
It's not the same. The task manager numbers keep changing. I watch it live, the percentages of the task managers keep changing.
```
